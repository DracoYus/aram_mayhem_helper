...
@pytest.fixture
def crawler_env(monkeypatch: pytest.MonkeyPatch, fixture_data_dir, game_data):
    """爬虫测试环境：旧 config 单例 data_path 指向 fixture + get_game_data 注入。"""
...
class TestSaveToFile:
    def test_writes_json(self, crawler_env, monkeypatch) -> None:
        ...
        saved = json.loads((crawler_env / "opgg" / "aram_augments" / "42.json").read_text(encoding="utf-8"))
        assert saved == {"a": 1}
...
class TestAramkitCrawler:
    ...
    def test_discover_versions_from_html(self, crawler_env, monkeypatch) -> None:
        ...
        cached = json.loads((crawler_env / "aramkit" / "version.json").read_text(encoding="utf-8"))
    ...
    def test_discover_versions_falls_back_to_cache(self, crawler_env, monkeypatch) -> None:
        crawler = self._make(crawler_env, monkeypatch)
        (crawler_env / "aramkit").mkdir(exist_ok=True)
        (crawler_env / "aramkit" / "version.json").write_text(
    ...
    def test_fetch_resources_saves_to_version_dir(self, crawler_env, monkeypatch) -> None:
        ...
        aug_file = crawler_env / "aramkit" / "resources" / "16.0.1-abc123456789" / "augments.json"
...
class TestChampionCrawler:
    def test_crawl_fetches_latest_version(self, crawler_env, monkeypatch) -> None:
        ...
        saved = json.loads((crawler_env / "ddragon" / "champions" / "16.0.1.json").read_text(encoding="utf-8"))
        assert saved == {"data": {}}
