...
def _patch_crawler_data(monkeypatch: pytest.MonkeyPatch, patch_config_data_path) -> Data:
    """把爬虫模块引用的 data 单例替换为 fixture 数据源。"""
    fixture_data = Data()
    monkeypatch.setattr(opgg_mod, "data", fixture_data)
    monkeypatch.setattr(aramkit_mod, "data", fixture_data)
    return fixture_data


def _make_opgg_crawler(patch_config_data_path, monkeypatch) -> AramAugmentCrawler:
    _patch_crawler_data(monkeypatch, patch_config_data_path)
...
class TestFetchJson:
    def test_success_returns_payload(self, patch_config_data_path, monkeypatch) -> None:
        crawler = _make_opgg_crawler(patch_config_data_path, monkeypatch)
        crawler.session = FakeSession(responses={"http://x": FakeResponse(payload={"ok": 1})})
        assert crawler.fetch_json("http://x") == {"ok": 1}

    def test_json_decode_error_returns_none(self, patch_config_data_path, monkeypatch) -> None:
        crawler = _make_opgg_crawler(patch_config_data_path, monkeypatch)
    ...
    def test_http_error_returns_none(self, patch_config_data_path, monkeypatch) -> None:
        crawler = _make_opgg_crawler(patch_config_data_path, monkeypatch)
        crawler.session = FakeSession(responses={"http://x": FakeResponse(payload={}, error=requests.HTTPError("404"))})
        assert crawler.fetch_json("http://x") is None

    def test_request_exception_is_swallowed_inside(self, patch_config_data_path, monkeypatch) -> None:
...
class TestSaveToFile:
    def test_writes_json(self, patch_config_data_path, monkeypatch) -> None:
        crawler = _make_opgg_crawler(patch_config_data_path, monkeypatch)
        assert crawler.save_to_file({"a": 1}, "42") is True
        saved = json.loads((patch_config_data_path / "opgg" / "aram_augments" / "42.json").read_text(encoding="utf-8"))
        assert saved == {"a": 1}


class TestCrawlAndSave:
    def test_success_chain(self, patch_config_data_path, monkeypatch) -> None:
        crawler = _make_opgg_crawler(patch_config_data_path, monkeypatch)
        crawler.session = FakeSession(responses={"http://x": FakeResponse(payload={"a": 1})})
        assert crawler.crawl_and_save("http://x", "42") is True

    def test_fetch_failure_returns_false(self, patch_config_data_path, monkeypatch) -> None:
        crawler = _make_opgg_crawler(patch_config_data_path, monkeypatch)
        crawler.session = FakeSession(responses={"http://x": FakeResponse(payload={}, json_error=True)})
...
class TestBatchCrawl:
    def test_crawls_fixture_champions(self, patch_config_data_path, monkeypatch) -> None:
        crawler = _make_opgg_crawler(patch_config_data_path, monkeypatch)
        session = FakeSession()
        crawler.session = session
    ...
    def test_stops_after_10_consecutive_failures(self, patch_config_data_path, monkeypatch) -> None:
        crawler = _make_opgg_crawler(patch_config_data_path, monkeypatch)
...
class TestAramkitCrawler:
    def _make(self, patch_config_data_path, monkeypatch) -> AramkitCrawler:
        _patch_crawler_data(monkeypatch, patch_config_data_path)
        crawler = AramkitCrawler()
        monkeypatch.setattr(time, "sleep", lambda s: None)
        return crawler

    def test_discover_versions_from_html(self, patch_config_data_path, monkeypatch) -> None:
        crawler = self._make(patch_config_data_path, monkeypatch)
        html = (
        ...
        cached = json.loads((patch_config_data_path / "aramkit" / "version.json").read_text(encoding="utf-8"))
        assert cached["data_version"] == "16.15-20260805-7e30d3443ba1"

    def test_discover_versions_falls_back_to_cache(self, patch_config_data_path, monkeypatch) -> None:
        crawler = self._make(patch_config_data_path, monkeypatch)
        (patch_config_data_path / "aramkit").mkdir(exist_ok=True)
        (patch_config_data_path / "aramkit" / "version.json").write_text(
    ...
    def test_discover_versions_raises_without_source(self, patch_config_data_path, monkeypatch) -> None:
        crawler = self._make(patch_config_data_path, monkeypatch)
        monkeypatch.setattr(crawler, "fetch_text", lambda url: None)
        with pytest.raises(RuntimeError, match="无法发现 aramkit 数据版本"):
            crawler.discover_versions()

    def test_fetch_resources_saves_to_version_dir(self, patch_config_data_path, monkeypatch) -> None:
        crawler = self._make(patch_config_data_path, monkeypatch)
        crawler.fetch_json = lambda url: {"1001": {"name": "泰坦的坚决"}}  # type: ignore[method-assign]
        crawler.fetch_resources("16.0.1-abc123456789")
        aug_file = patch_config_data_path / "aramkit" / "resources" / "16.0.1-abc123456789" / "augments.json"
        assert json.loads(aug_file.read_text(encoding="utf-8")) == {"1001": {"name": "泰坦的坚决"}}

    def test_crawl_sets_data_version_and_uses_it(self, patch_config_data_path, monkeypatch) -> None:
        crawler = self._make(patch_config_data_path, monkeypatch)
...
class TestChampionCrawler:
    def test_crawl_fetches_latest_version(self, patch_config_data_path, monkeypatch) -> None:
        monkeypatch.setattr(time, "sleep", lambda s: None)
        crawler = ChampionCrawler()
        crawler.fetch_json = lambda url, params=None: {"data": {}}  # type: ignore[method-assign]
        crawler.get_latest_ddragon_version = lambda: "16.0.1"  # type: ignore[method-assign]
        assert crawler.crawl() is True
        saved = json.loads(
            (patch_config_data_path / "ddragon" / "champions" / "16.0.1.json").read_text(encoding="utf-8")
        )
        assert saved == {"data": {}}
