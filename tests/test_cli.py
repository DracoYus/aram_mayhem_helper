...
class TestRecommend:
    """CLI recommend 入口的展示层行为（业务流程在 test_recommend_flow.py 中覆盖）。"""

    def test_prints_lines_via_stdout(self, monkeypatch, capsys) -> None:
        import aram_mayhem_helper.algorithm.recommend_flow as recommend_flow

        outcome = recommend_flow.RecommendOutcome(lines=["快选符文：泰坦的坚决"])
        monkeypatch.setattr(cli, "get_game_data", lambda: object())
        monkeypatch.setattr(cli, "get_ocr_tool", lambda: object())
        monkeypatch.setattr(cli, "run_recommend", lambda *args, **kwargs: outcome)
        cli.recommend()
        assert capsys.readouterr().out == "快选符文：泰坦的坚决\n"

    def test_no_lines_prints_nothing(self, monkeypatch, capsys) -> None:
        import aram_mayhem_helper.algorithm.recommend_flow as recommend_flow

        outcome = recommend_flow.RecommendOutcome(lines=[])
        monkeypatch.setattr(cli, "get_game_data", lambda: object())
        monkeypatch.setattr(cli, "get_ocr_tool", lambda: object())
        monkeypatch.setattr(cli, "run_recommend", lambda *args, **kwargs: outcome)
        cli.recommend()
