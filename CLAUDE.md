...
│   └── overlay.py      # 推荐结果悬浮窗 (Tkinter 置顶无边框, 截图排除, 最短 8s 自动关闭——选择界面仍在则保活; CLI 降级为控制台打印)
├── algorithm/
