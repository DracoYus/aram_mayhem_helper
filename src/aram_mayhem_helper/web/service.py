...
def build_champion_augments(game_data: GameData, champion_id: str, source: str | None = None) -> list[dict]:
...
def build_champion_list(game_data: GameData, source: str | None = None) -> list[dict]:
    """返回所有已缓存符文数据的英雄摘要列表。"""
    source = source or game_data.default_source()
    champions: list[dict] = []
